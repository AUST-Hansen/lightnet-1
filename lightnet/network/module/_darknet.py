    ...
    def _load_darknet_weights(self, weights_file):
        ...
        for module in self.modules_recurse():
            try:
                ...
                log.debug('Layer loaded: {module}'.format(module=module))
    ...
    def _save_darknet_weights(self, weights_file, seen=None):
        ...
        for module in self.modules_recurse():
            try:
                ...
                log.debug('Layer saved: {module}'.format(module=module))
            except NotImplementedError:
                log.debug('Layer skipped: {module.__class__.__name__}'.format(module=module))
...
class WeightLoader:
    ...
    def __init__(self, filename):
        with open(filename, 'rb') as fp:
            ...
            log.debug('Loading weight file: version {header0}.{header1}.{header2}'.format(
                header0=self.header[0],
                header1=self.header[1],
                header2=self.header[2],
            ))
    ...
    def load_layer(self, layer):
        ...
        if type(layer) == nn.Conv2d:
            ...
            raise NotImplementedError('The layer you are trying to load is not supported [{layer}]'.format(layer=type(layer)))
...
class WeightSaver:
    ...
    def write_file(self, filename):
        ...
        log.debug('Writing weight file: version {header0}.{header1}.{header2}'.format(
            header0=self.header[0],
            header1=self.header[1],
            header2=self.header[2],
        ))
    ...
    def save_layer(self, layer):
        ...
        if type(layer) == nn.Conv2d:
            ...
            raise NotImplementedError('The layer you are trying to save is not supported [{layer}]'.format(layer=type(layer)))
